...
export const BUILT_IN_TUTORIAL_VERSION = 5.0;
...
读点书是一个和 AI 角色一起阅读的工具。你可以导入书籍、配置 AI 接口，让你选定的角色陪你一起读书、发表评论、回答问题，也可以写笔记、做测验。

所有数据（书籍、聊天记录、笔记、设置等）均存储在浏览器本地。建议定期在设置的「存储管理」中导出存档文件备份，换设备或清除浏览器数据前一定要先备份。

主要功能一览：

一、导入书籍
支持 TXT、EPUB、PDF、DOCX、MOBI 格式，也可以粘贴网络链接或手动输入文字。

二、配置 AI 接口
支持 OpenAI、DeepSeek、Gemini、Anthropic Claude 及兼容 OpenAI 格式的自定义接口，可保存多套预设方便切换。

三、和角色一起阅读
在阅读界面点击右上角的对话图标打开聊天窗口，角色会根据当前阅读内容和上下文与你互动。
可发消息召唤角色回复，也可以在设置里开启「主动发送消息」，让角色自动在你阅读过程中发言。

四、高亮与标注
选中正文文字可以高亮标注，支持六种颜色，角色能感知你高亮的内容。
也可以开启「主动高亮」，让角色在喜欢的片段自动添加下划线。

五、笔记与问答
在「共读集」中可以创建笔记本、写读书笔记，并召唤角色对笔记进行评论。
还可以基于书籍内容生成测验题目，答完后角色会给出评价。

六、成就印章
角色在特定对话中可能会颁发成就，记录在首页的「印章」中。

七、阅读统计
记录每天的阅读时长、连续阅读天数、读完书目等，支持设定阅读目标。

八、TTS 朗读
可配置文字转语音接口，在阅读界面逐段朗读正文内容。`;

const CH0_BLOCKS: ReaderContentBlock[] = [
  text('读点书 · 使用说明'),
  text(`读点书是一个和 AI 角色一起阅读的工具。你可以导入书籍、配置 AI 接口，让你选定的角色陪你一起读书、发表评论、回答问题，也可以写笔记、做测验。
